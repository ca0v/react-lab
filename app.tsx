  ...
  render(): any {
    return (
      <div className="app">
        ...
        {!this.state.featureNameFieldName && (
          <Toolbar>
            {Object.keys(packets)
              .sort(
                (a, b) =>
                  storage.force(b).score - storage.force(a).score ||
                  a.localeCompare(b)
              )
              .map((p) => (
                <button onClick={() => this.pickPacket(p)}>
                  {p} ({storage.force(p).score})
                </button>
              ))}
          </Toolbar>
        )}
        {!!this.state.packetName && (
          <QuizletComponent
            questionsPerQuiz={20}
