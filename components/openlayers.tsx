...
export type OtherImagerySet =
  | "Fast"
  | "WaterColor"
  | "WaterColorWithLabels"
  | "Black"
  | "BlackWithLabels"
  | "EsriAerial"
  | "EsriAerialWithLabels"
  | "CanvasDarkWithLabels";
...
export class OpenLayers extends Component<OpenLayersProps, OpenLayersState> {
  ...
  constructor(props: OpenLayersProps) {
    ...
    this.dispatcher.on("ensure-extent-visible", (args: { extent: Extent }) => {
      ...
      const finalExtent = extent.buffer(
        extent.extend(currentExtent, targetExtent),
        0.2 * extent.getWidth(currentExtent)
      );
  ...
  render() {
        ...
        DrawControls = () => (
          <Toolbar>
            {draw.point && (
              <button
                className={`${
                  this.state.activeDrawingTool === "Point"
                    ? "active"
                    : "inactive"
                }`}
                onClick={() => map && this.draw("Point")}
              >
                Point
              </button>
            )}
            {draw.circle && (
              <button
                className={`${
                  this.state.activeDrawingTool === "Circle"
                    ? "active"
                    : "inactive"
                }`}
                onClick={() => map && this.draw("Circle")}
              >
                Circle
              </button>
            )}
            {draw.line && (
              <button
                className={`${
                  this.state.activeDrawingTool === "LineString"
                    ? "active"
                    : "inactive"
                }`}
                onClick={() => map && this.draw("LineString")}
              >
                Line
              </button>
            )}
            {draw.polygon && (
              <button
                className={`${
                  this.state.activeDrawingTool === "Polygon"
                    ? "active"
                    : "inactive"
                }`}
                onClick={() => map && this.draw("Polygon")}
              >
                Polygon
              </button>
            )}
    ...
    return (
      <div
        className={`maplet ${this.props.className || ""} ${
      ...
      .forEach((interaction) => {
        if (interaction instanceof ol.interaction.Draw) {
          interaction.setActive(
            interaction.get("type") === this.state.activeDrawingTool
          );
        }
      });
  }
