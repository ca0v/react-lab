...
export class FeatureAnimation extends Object {
  constructor(public options: FeatureAnimationOptions) {
    ...
    let easing =
      typeof options.easing == "function" ? options.easing : ol.easing.linear;
    options.easing = options.revers ? (t: number) => 1 - easing(t) : easing;
  }

  /** Draw a geometry
   */
  drawGeom_(e: any, geom: Geometry, shadow: Geometry) {
    ...
    for (let i = 0; i < style.length; i++) {
      ...
      let imgs = Map.prototype.getFeaturesAtPixel ? false : style[i].getImage();
      if (imgs) {
        sc = imgs.getScale();
...
export class ExplodeAnimation extends FeatureAnimation {
  ...
  constructor(
    public options: ExplodeAnimationOptions & FeatureAnimationOptions
  ) {
    super(options);
    ...
    let gradient = (this.gradient = ctx.createRadialGradient(
      0,
      0,
      0,
      0,
    ...
    gradient.addColorStop(
      0,
      "rgba(" + [mask(r, 255), mask(g, 255), mask(b, 255), a] + ")"
    );

    for (var i = 0; i < options.length; i++) {
      gradient.addColorStop(
        i / options.length,
        `rgba(${mask(r, 255 - Math.random() * 100)}, ${mask(
          g,
          255 - Math.random() * 200
        )}, ${mask(b, 255 - Math.random() * 100)}, ${1 - i / options.length})`
      );
    }
...
export function explode(layer: Vector, coord: Coordinate) {
  var f = new Feature(new ol.geom.Point(coord));
